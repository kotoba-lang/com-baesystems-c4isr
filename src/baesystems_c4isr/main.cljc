(ns baesystems_c4isr.main
  "Kotodama WASM entrypoint for the Baesystems_c4isr clean-room actor (L4) — Clojure port.
  ...
  facts (`baesystems_c4isr.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "baesystems_c4isr")
(def tier "L4")
...
(def entity-specs
  [{:entity "Case"      :plural "cases"      :id-prefix "baesyste_cas"
    :fields [:number :jurisdiction :status :filedAt] :required [:number :jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "baesyste_par"
    :fields [:caseId :role :name]          :required [:role :name]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Document"  :plural "documents"  :id-prefix "baesyste_doc"
    :fields [:caseId :type :contentRef]          :required [:type]
    :coerce {}                       :refs {:caseId "Case"}}
   {:entity "Incident"  :plural "incidents"  :id-prefix "baesyste_inc"
    :fields [:type :jurisdiction :occurredAt]        :required [:type :jurisdiction]
    :coerce {}          :refs {}}
   {:entity "Watchlist" :plural "watchlists" :id-prefix "baesyste_wat"
    :fields [:name :program :entries]          :required [:name :program]
    :coerce {:entries :int}          :refs {}}
   {:entity "Sanction"  :plural "sanctions"  :id-prefix "baesyste_san"
...
(defn emit-facts
  "EAVT facts for one record: {\"baesystems_c4isr.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "baesystems_c4isr-compat" :tier tier :entities entities} 200])
...
(defn start! [] :baesystems_c4isr-compat/ready)
